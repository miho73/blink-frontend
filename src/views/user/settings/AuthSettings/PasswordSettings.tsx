...
function PasswordSettings() {
  ...
  function passwordFormInvalid(flag: number) {
    setFormState(flag);
    setWorking(false)
  }
  ...
  if(!changingPwd) {
    return (
      <>
        <Button className={'w-fit'} onClick={() => setChangingPwd(true)}>비밀번호 변경</Button>
        {checkFlag(formState, 0) && <Alert variant={'success'}>암호가 변경되었습니다.</Alert>}
      </>
    );
  }
  else {
    return (
      <Stack>
        ...
        <Stack direction={'row'} className={'gap-3 mt-2'}>
          <Button className={'w-fit'} onClick={cancelChangePassword} disabled={working}>취소</Button>
          <Button color={'accent'} className={'w-fit'} onClick={startChangePassword} disabled={working}>암호 변경</Button>
        </Stack>
